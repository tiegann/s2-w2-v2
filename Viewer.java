...
public class Viewer implements LineListener
{
	...
	// Simple actions menu for invoking transformations from the viewer
	...
	private static int controlHeight = 100;
	...
	/*
	 * Return the maximum sample value for the current audio format.
	 */
	public int getMaxSampleValue() {
	    return (int) myHelper.getMaxSampleValue();
	}
	...
	public ArrayList<Integer> load() {
		...
		try {
			if (soundsDir.exists() && soundsDir.isDirectory()) {
				fileChooser = new JFileChooser(soundsDir.getCanonicalPath());
			} else {
				File thisFile = new File(".");
				fileChooser = new JFileChooser(thisFile.getCanonicalPath());
			}
		} catch (Exception e) {
			System.out.println("Cannot find sounds directory.");
	...
	/*
	 * Load sound data from a file without creating a new viewer frame.
	 * Used for switching sounds in an existing viewer.
	 */
	public ArrayList<Integer> loadDataOnly(String filename) {
		if (filename == null) {
			System.err.println("Please specify a valid file name");
			return null;
		}
		...
		data = myHelper.getData();
		zoomReset();
		// Reset controls to a clean idle state and refresh layout
		resetControlsAfterLoad();
		// Refresh views to reflect the newly loaded data
		wavePanel.refresh(false);
		overViewPanel.refresh();
		return data;
	}

	// Reset control panel buttons and state after loading/switching sounds
	private void resetControlsAfterLoad() {
		if (playBtn != null) playBtn.setEnabled(true);
		if (stopBtn != null) stopBtn.setEnabled(false);
		if (pauseBtn != null) pauseBtn.setEnabled(false);
		if (playSelectionBtn != null) playSelectionBtn.setEnabled(false);
		if (zoomInBtn != null) zoomInBtn.setEnabled(false);
		if (zoomOutBtn != null) zoomOutBtn.setEnabled(false);
		playSelClicked = 0;
		isPlaying = false;
	...
	/*
	 * Returns the underlying Helper for advanced usages like cloning.
	 */
	...
	/*
	 * Set the Sound instance this viewer is working with.
	 * Used to enable transformation replay when loading new sounds.
	 */
	...
	/*
	 * Open this viewer using an existing Helper (audio format and buffer).
	 * Initializes panels and UI, displays the frame.
	 */
	public void openWithHelper(Helper helper) {
		myHelper = helper;
		myHelper.setViewer(this);
		data = myHelper.getData();
		createMainFrame();
	...
	/*
	 * Returns the current data reference used by the viewer.
	 * This allows callers to detect if the underlying list object
	 * has been replaced and sync accordingly.
	 */
	...
	/*
	 * Register a user-defined action (e.g., a Sound transformation) in the viewer UI.
	 * When invoked, the action is executed and the viewer refreshes.
	 */
	public void addAction(String name, Runnable action) {
		if (menuBar == null || actionsMenu == null) return;
		JMenuItem item = new JMenuItem(name);
		item.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					// Record the transformation for replay when switching sounds
					if (mySound != null) {
						mySound.addTransformationToHistory(action);
					}
					action.run();
				} finally {
					refresh(true);
	...
	private void createMainFrame() {
		...
		// Setup a basic menu bar for actions
		menuBar = new JMenuBar();
		actionsMenu = new JMenu("Actions");
		menuBar.add(actionsMenu);
		
		// Add Sounds menu for loading different sound files
		soundsMenu = new JMenu("Sounds");
		populateSoundsMenu();
	...
	/*
	 * Populate the Sounds menu with all audio files from the sounds/ directory.
	 * Clicking a sound file loads it and replays all transformations.
	 */
	private void populateSoundsMenu() {
		File soundsDir = new File("sounds");
		if (!soundsDir.exists() || !soundsDir.isDirectory()) {
			return;
		}

		File[] soundFiles = soundsDir.listFiles((dir, name) -> {
			String lower = name.toLowerCase();
			return lower.endsWith(".wav") || lower.endsWith(".au") || 
			       lower.endsWith(".aif") || lower.endsWith(".aiff");
		});
		...
		for (File soundFile : soundFiles) {
			JMenuItem item = new JMenuItem(soundFile.getName());
			item.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (mySound != null) {
						mySound.loadAndApplyTransformations(soundFile.getPath());
					}
				}
			});
			soundsMenu.add(item);

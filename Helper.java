...
public class Helper implements LineListener{
	...
	public void setData(ArrayList<Integer> data) {
	   this.data = data;
	   // Align the reported frame length with the new data size to keep bounds checks consistent
	   if (audioFileFormat != null) {
		   audioFileFormat = new AudioFileFormat(
	 ...
	 */
	public void refresh() {
	   // Ensure buffer length matches frames * frameSize (accounts for channels)
	   int frameSize = audioFileFormat.getFormat().getFrameSize();
	   if (viewer.DEBUG)
		  System.err.println("Data size is " + data.size() +  "...  buffer length is " + buffer.length + "...  frameSize is " + frameSize);
	   if (buffer.length != (data.size() * frameSize)) {
			buffer = new byte[data.size() * frameSize];
			viewer.zoomReset();
		}
